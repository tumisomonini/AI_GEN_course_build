...
MODEL = 'gpt-4o-mini'  # Assume common model; update as needed
ENC = tiktoken.encoding_for_model(MODEL)
COST_PER_1M_INPUT = 0.15 / 1_000_000  # $ example
...
def cost_estimate(input_tokens, output_tokens):
    cost = (input_tokens * COST_PER_1M_INPUT + output_tokens * COST_PER_1M_OUTPUT) / 1_000  # to $
    score = max(0, 10 - cost * 10)  # scale, <1$ =10
...
async def test_api_generate(pg):
    ...
    timings = []
    results = []
    for i in range(10):  # 10 iterations for robust stats
        start = time.time()
    ...
    for i in range(5):  # 5 iterations for robust stats
        start = time.time()
    ...
    # Ensure DB health before tests (fixes Docker + connect issues)
    ...
    pg = init_postgres()  # Ensures Docker up + schema
    
    print("🚀 Initializing Neo4j...")
    neo4j_repo = init_neo4j()  # Ensures Docker up + connect
    neo4j_repo.close()
    ...
    # Calculate Dynamic Scores for the 5 Key Factors
