...
# Ensure env vars are available even when AuthorAgent is instantiated outside the API server.
...
class AuthorAgent:
    def __init__(self, manager=None, repo=None, kg=None):
        # Fallback: if keys are not present yet, attempt to load .env from repo root.
        # This prevents "No valid LLM API key found" when AuthorAgent is created in tests/CLI.
        if not os.getenv("MISTRAL_API_KEY") and not os.getenv("OPENROUTER_API_KEY"):
            load_root_env()

        self.manager = manager
        self.vector_store = manager.astra.vector_store if manager and manager.astra else None
        ...
        raise ValueError(
            "No valid LLM API key found. Set MISTRAL_API_KEY (preferred) or OPENROUTER_API_KEY in .env"
        )
    ...
    async def generate_content(
        self,
        topic: str,
        state: Optional[SyllabusState] = None,
        order: int = 0,
        reviewer=None,
        max_retries: int = 1,
    ) -> Optional[Chapter]:
        ...
        if reviewer:
            ...
            if self.repo and run_id:
                self.repo.log_message(
                    run_id, "AuthorAgent", f"Routing strategy: {strategy} ({route_data.get('reason')})"
                )
        ...
        grounding_instruction = (
            "Use ONLY the following scraped knowledge chunks as factual source material." if chunks else
            "Use your internal knowledge to provide accurate educational content."
        )
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if reviewer and attempt < max_retries:
                    # Pass the chunks to the reviewer for a faithfulness check
                    critique = await reviewer.validate_content_with_llm(content, topic, source_chunks=chunks)
                    if critique.get("semantic_pass"):
                        ...
                        if self.repo and run_id:
                            self.repo.log_message(
                                run_id,
                                "AuthorAgent",
                                f"Attempt {attempt + 1} failed review ({critique.get('reason', 'Low score')}). Feedback: {critique.get('feedback')}",
                                "warning",
                            )
                        # Add specific feedback to the next prompt
                        current_prompt = (
                            f"{prompt}\n\nREFINEMENT NEEDED: {critique.get('feedback')}. "
                            f"Focus strictly on the provided source chunks and resolve these issues: "
                            f"{critique.get('hallucinations', 'factual accuracy')}."
                        )
                        continue
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                chapters.append(
                    Chapter(
                        title=topics[i],
                        content="Generation failed: unknown error",
                        chapter_order=i,
                        status="error",
                    )
                )
